...
│   ├── notifications.ts     # Service-worker / Notification API wrappers
...
└── scripts/check-db.mjs     # One-off DB connectivity check
```
...
This is the most misunderstood part of the app, so it gets its own diagram. **The in-app reminder loop only runs while a ClassPing tab is open.** There is no push server, no cron, no Web Push subscription. That limitation is precisely why calendar export exists: the `.ics` file hands the reminders to the phone's *native* calendar, which fires them with the app closed.
...
| **A user says "reminders don't fire when the app is closed"** | Working as designed. The in-app loop needs an open tab. Direct them to Pro's calendar export, which hands reminders to the native calendar. |
...
| --- | --- | --- | --- |
| 1 | **Nothing routes the user to `/prompt`.** The screen now correctly identifies the class that just ended, but no code navigates there when a class ends. It's reachable only by typing the URL. | `lib/store.tsx` reminder loop | The post-class nudge — a headline feature — effectively never fires. The reminder loop already detects class boundaries every 30s; wiring an end-of-class notification that deep-links to `/prompt` is the natural fix. |
| 2 | **`npm run lint` is not configured.** The script exists, but ESLint was never set up — running it drops into an interactive "How would you like to configure ESLint?" prompt and lints nothing. | `package.json` | **The repo has no linter.** Fix with `npm i -D eslint eslint-config-next` and an `eslint.config.mjs`, or drop the script so it stops implying coverage that doesn't exist. |
| 3 | **API routes 307-redirect unauthenticated callers** to the sign-in page instead of returning JSON. The `401` branches in both route handlers are unreachable from a browser, because `middleware.ts` intercepts first. | `middleware.ts` | Harmless today (the client only calls these when signed in), but it makes the API unusable to any non-browser client and the `401` code paths are dead. |
| 4 | **Grades gate is client-only.** See §1.2. Accepted: grades never touch the server on the free plan, so there's no server resource to protect and nothing to enforce. | `app/grades/page.tsx` | Low. Documented, not a bug. |
| 5 | **No tests.** | — | Start with `lib/gpa.ts` and `lib/calendar.ts`: pure functions, no mocking, and the highest cost of silent breakage — a wrong GPA or a malformed `.ics` both destroy trust. |
